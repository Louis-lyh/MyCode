...
namespace _1_Code.AStart
{
    ...
    public static class PathFinding
    {
        ...
        public static  List<int> Dijkstra(MapItemType[][] mapData,int start,int end,out Dictionary<int,int> cameFrom)
        {
            ...
            // 需要检查的点
            CustomPriorityQueue<int,int> frontier = new CustomPriorityQueue<int,int>();
        ...
        /// </summary>
        /// <param name="mapData"></param>
        public static  List<int> AStart(MapItemType[][] mapData,int start,int end,out Dictionary<int,int> cameFrom)
        {
            // 搜索方向
            int[][] dir = new int[4][] {new []{0,1},new []{1,0},new []{-1,0},new []{0,-1} };
                ...
                for (int i = 0; i < dir.Length; i++)
                {
                        ...
                        && mapData[nextX][nextY] != MapItemType.OBS)
                    {
                        ...
                        if (!cameFrom.ContainsKey(next) || newCost < costSoFar[next])
                        {
                            // 更新移动成本
                            costSoFar[next] = newCost;
                            var priority = newCost + Heuristic(end % mapData.Length,end / mapData.Length,nextX,nextY);
        ...
        /// </summary>
        /// <returns></returns>
        private static int Heuristic(int endX, int endY,int currentX,int currentY)
        {
